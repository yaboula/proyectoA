...
<template>
  <KioskLayout>
    <header class="glass-panel kiosk-panel rounded-md px-6 py-5">
      <div class="gcma-toolbar">
        <div class="min-w-0">
          <div class="gcma-section-label">Pilotage atelier</div>
          <h1 class="mt-2 text-3xl font-black tracking-tight text-white md:text-4xl">Ordres de fabrication</h1>
        ...
        <div class="flex flex-wrap gap-2 shrink-0">
          <button @click="router.push({ name: 'hub' })"
                  class="h-12 px-4 flex items-center gap-2 rounded-md border border-zinc-800 bg-zinc-950 text-zinc-50 text-sm font-semibold active:bg-zinc-900 transition">
            <LayoutDashboard :size="18" />
            Modules
          </button>
          <button v-if="store.hasModule('quality')" @click="router.push({ name: 'laboratoire' })"
                  class="h-12 px-4 flex items-center gap-2 rounded-md border border-zinc-800 bg-zinc-950 text-zinc-300 text-sm font-semibold active:bg-zinc-900 transition">
            <FlaskConical :size="18" />
            Laboratoire
          </button>
          <button @click="fetchTareas"
                  :disabled="loading"
                  class="h-12 w-12 flex items-center justify-center rounded-md border border-zinc-800 bg-zinc-950 text-zinc-400 active:bg-zinc-900 disabled:opacity-30 transition">
            <RefreshCw :size="20" :class="{ 'animate-spin': loading }" />
          </button>
          <button @click="logout"
                  class="h-12 px-4 flex items-center gap-2 rounded-md border border-zinc-800 bg-zinc-950 text-zinc-400 text-sm font-semibold active:bg-zinc-900 transition">
            <LogOut :size="18" />
            Quitter
          </button>
        </div>
      ...
      <div class="mt-5 grid gap-3 md:grid-cols-3 text-sm text-zinc-400">
        <div class="gcma-stat">
          <div class="gcma-section-label">Site</div>
          <div class="mt-1 font-semibold text-white">{{ store.operario?.company }}</div>
        </div>
        <div class="gcma-stat">
          <div class="gcma-section-label">Profil</div>
          <div class="mt-1 font-semibold text-white">{{ store.profileLabel }}</div>
        </div>
        <div class="gcma-stat">
          <div class="gcma-section-label">Ordres visibles</div>
          <div class="mt-1 font-semibold text-white">{{ tareas.length }}</div>
        </div>
      </div>
    </header>
    ...
    <main class="flex-1 space-y-4 overflow-y-auto">
      ...
      <div v-else-if="error" class="kiosk-panel flex flex-col items-center justify-center rounded-md py-28 gap-5">
        ...
        <button @click="fetchTareas"
                class="h-14 px-8 rounded-md bg-zinc-50 border border-zinc-50 text-zinc-900 text-base font-semibold active:bg-zinc-200 transition">
          Reessayer
        </button>
      </div>
        ...
        <article v-for="t in tareas" :key="t.work_order" class="kiosk-panel overflow-hidden rounded-md">
          <div class="grid gap-4 p-5 xl:grid-cols-[1.2fr_0.9fr_auto] xl:items-center">
            <div class="space-y-4">
              ...
              <div>
                <h2 class="text-2xl font-black leading-tight text-zinc-50">{{ t.producto }}</h2>
                <div class="mt-3 flex items-baseline gap-2">
                  <span class="text-4xl font-black text-zinc-50">{{ t.cantidad_pendiente }}</span>
                  <span class="text-lg text-zinc-400">{{ t.uom }}</span>
                  <span class="text-sm text-zinc-500 ml-1">a produire</span>
                </div>
              </div>
            </div>
            ...
            <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-1">
              <div class="gcma-stat">
                ...
                <div class="mt-2 flex items-center gap-2 text-sm font-bold"
                     :class="materialsReady(t) ? 'text-zinc-50' : 'text-amber-300'">
              ...
              <div class="gcma-stat">
                ...
                <div class="mt-2 text-sm font-semibold text-zinc-50">{{ t.materiales?.length ?? 0 }} materiaux</div>
            ...
            <div class="flex xl:justify-end">
              <button @click="startProduction(t.work_order)"
                      class="w-full h-16 min-w-[16rem] rounded-md bg-zinc-50 px-6 text-base font-black tracking-[0.12em] text-zinc-900 active:bg-zinc-200 transition-colors xl:w-auto">
